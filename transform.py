...
if __name__ == "__main__":
    ...
    if not (os.path.isfile(sys.argv[1])):
        ...
        exit(0)
    if not (os.path.isfile("generator_latest.pth")):
        print(
            "Can not find pre-trained weights file generator_release.pth. Please provide within current directory."
        )
        exit(0)
    checkpoint = torch.load("./generator_latest.pth", map_location="cpu")
    G = Generator().to("cpu")
    ...
    with Image.open(sys.argv[1]) as img:
        ...
        result.save(name + "_transformed." + img.format)
